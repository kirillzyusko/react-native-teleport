...
After extensive development, community feedback and integrating the lib into many prod apps, we've reached a big milestone: a **production-ready** portal solution that brings **true native portals** to react-native — something the framework has been missing for years.
...
Imagine you have the same component rendered twice on two different screens. If it's heavy for rendering a typical path is to dig into performance bottlenecks, fixing them etc. But with `react-native-teleport` you can **simply move** the component between screens and it will be rendered only once 🤯

### And more

I'm still exploring of what can be done with this library. Things that I discovered helped to build flicker free transitions while moving heavy views and helped to optimize performance from the angle that I never thought about that (just re-using the view without re-creating it).
...
- 🪞 Mirror support for advanced mirroring patterns
...
- ⭐ Starring on [GitHub](https://github.com/kirillzyusko/react-native-teleport)
