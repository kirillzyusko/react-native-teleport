...
The `react-native-teleport` solves this problem by moving rendering of components in the native layer. This library provides a **declarative API**, doesn't rely on **_internal_** React implementation details, and **works identically** on **iOS, Android, and Web**.
...
<Video src="/video/instagram.mp4" width={35} />
<br />
...
Imagine you have the same component rendered twice on two different screens. If it's heavy for rendering a typical path is to dig into performance bottlenecks, fixing them etc. But with `react-native-teleport` you can **simply move** the component between screens 🤯
