...
# Building Instagram-like Shared Transitions
...
- [`@react-navigation/native-stack`](https://reactnavigation.org/docs/native-stack-navigator/) — for navigation
- [`react-native-reanimated`](https://docs.swmansion.com/react-native-reanimated/) — for smooth animations
- [`react-native-video`](https://docs.thewidlarzgroup.com/react-native-video) — for video playback
...
A simple scrollable list of posts:
...
1. **Visible background during transitions** — When the video animates from its card position to full-screen, the feed is still visible behind it. A regular screen push would show a solid background.
...
At this point you have a working app — tapping a video opens Reels, pressing back returns to Feed. But there's no transition animation; the screen simply appears and disappears. Let's fix that.
...
:::info Why Zustand?
We use Zustand instead of React Context because it gives us **granular subscriptions**. Each component selects only the slice of state it needs (`state.id === post.id`, `state.progress`, etc.), so only the relevant components re-render. This is critical for performance during animations. Also, Zustand stores work seamlessly with Reanimated `SharedValue` — you can't store a `SharedValue` in React state, but you can in a Zustand store.
:::
