...
* [`react-native-teleport`](/react-native-teleport/pr-preview/pr-91/docs/installation.md) — the star of the show
...
1. **Visible background during transitions** — When the video animates from its card position to full-screen, the feed is still visible behind it. A regular screen push would show a solid background (which is not desired in out case).
...
At this point you have a working app — tapping a video opens Reels, pressing back returns to Feed, but:
...
The header animates alongside the video — it fades in and slides down from the video's original position:
